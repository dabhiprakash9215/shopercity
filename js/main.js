  ...
  Riode.scrollSet = function (selector) {
    ...
    Riode.$body.on("click", selector, function (e) {
      var $this = $(this),
        ...
        hash = link.hash ? link.hash : link.slice(link.lastIndexOf("#"));
      if (hash.startsWith("#")) {
    ...
    var initCompareAction = function (self) {
      ...
      self.$wrapper.on("click", ".btn-compare", function (e) {
        ...
        if (
          ($product.closest(".product-popup").length ||
            document.body.classList.contains("home")) &
          ($product.find(".btn-product.btn-cart").attr("disabled") !=
            "disabled")
        ) {
          Riode.Minipopup.open({
            message: "Successfully Added",
            ...
            .replace(/M([\d|\.]*),([\d|\.]*)/, function (match, p1, p2) {
              if (p1 && p2) {
                return (
                  ...
                  (
                    parseFloat(p2) +
                    (dy = self.getDeltaY((dx += parseFloat(p1))))
                  ).toFixed(3)
                );
              }
              return match;
        ...
        .on(
          ...
          function (e) {
            ...
            if ($compareCol.length > 0) {
              productPrice = $(".riode-compare-table > .compare-row")
                .eq(2)
                .children()
                .eq($compareCol.index())
                .find(".product-price .new-price, .product-price .price")
                .html();
              $productName = $(".riode-compare-table > .compare-row")
                .eq(1)
                .children()
                .eq($compareCol.index());
